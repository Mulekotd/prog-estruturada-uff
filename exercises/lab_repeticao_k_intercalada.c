...
int main()
{
    ...
    int currentLength = 0; // Contador para o tamanho do segmento atual
    int lastPair = -1;      // Começa em -1 para indicar que nenhum número foi lido ainda
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        else if (currentPair == lastPair)
        {
            ...
            if (k > 0 && currentLength > k) valid = 0;
        } 
        else
        {
    ...
    {
        if (currentLength != k) valid = 0; // O último segmento processado também precisa ter exatamente o tamanho k
    }
